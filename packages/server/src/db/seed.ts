...
const SEED_CATEGORIES = [
  ...
  { slug: "deep", name: "Deep", emoji: "🤔" },
  { slug: "bizarre", name: "Bizarre", emoji: "🌀" },
];
...
async function main() {
  ...
  const existing = await db.select({ value: count() }).from(questions);
  if ((existing[0]?.value ?? 0) > 0) {
    console.log(
      `Questions table already has ${existing[0]?.value} rows — skipping question seed.`,
    );
    process.exit(0);
  }
  ...
  if (!general) throw new Error("General category missing after seed");
